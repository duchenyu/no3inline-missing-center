...
The reconstructed full counts enabled a quantitative three-factor weighted least squares model (using $D_4$-inequivalent counts for $n=7$--$20$, with correct ring-pair counts $C(R,2)$ where $R$ is the number of distinct distance rings):
\[
\text{missing rate} = -1.22 + 0.77\cdot(\text{mod4}) + 2.95\cdot(\text{prime}) + 0.44\cdot\log C(R,2),
\]
with $R^2 = 0.834$ (adjusted $R^2 \approx 0.78$). The $\text{mod4}$ effect is modest ($+0.77$), while primality remains the strongest contributor ($+2.95$). Notably, the $\log C(R,2)$ coefficient is small and positive ($+0.44$), reversing the negative sign from earlier buggy data --- ring-pair density has negligible explanatory power once primality and parity are controlled.
...
18 & $\sim$152210 & $\sim$2735 & 1.80\% & Even \\
19 & $\sim$258170 & $\sim$18729 & 7.25\% & Odd \\
20 & $\sim$941580 & $\sim$18326 & 1.95\% & Even \\
...
A refined weighted least squares model incorporating $r_2$-based features achieves $R^2 = 0.897$ (with corrected $n=9$ data resolving earlier instability), a modest improvement over the three-factor model ($\Delta R^2 = +0.063$):
